...
@implementation FDDESUtility

+(NSData *)encryptWithData:(NSData *)data{
    //先加密,后Base64
    NSData *result=[FDDESUtility didOperation:data operation:kCCEncrypt];
    return [GTMBase64 encodeData:result];
}
+(NSData *)decryptWithData:(NSData *)data{
    //先Base64,后解密
    NSData *decodeBase64=[GTMBase64 decodeData:data];
    return [FDDESUtility didOperation:decodeBase64 operation:kCCDecrypt];
}
