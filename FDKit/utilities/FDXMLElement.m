...
@interface FDXMLElement () <NSXMLParserDelegate>{
    FDXMLElement          *parent;
...
@property (nonatomic,STRONG) FDXMLElement     *parent;
...
-(NSArray *) removeChildrenForName:(NSString *)elementName{
    ...
    if (array) {
        for (FDXMLElement *elem in array) {
            elem.parent = nil;
        }
    }
    return array;
...
-(void) each:(FDXMLElement *)root iterator:(FDXMLElementIterator) iterator level:(int)level{
    for (int k = 0; k<root.sortedChildElements.count; k++) {
        FDXMLElement *e = [root.sortedChildElements objectAtIndex:k];
        // invoke handler
        if(!iterator(e,level)){
            return;
        }
...
- (void)parserDidEndDocument:(NSXMLParser *)parser{
    ...
    FDXMLElement *root = [self.stack lastObject];
    if (root) {
...
-(void) copyFrom:(FDXMLElement *)other{
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{
    ...
    FDXMLElement *elem = AutoRelease([[FDXMLElement alloc] init]);
    
    elem.name = elementName;
    [elem.attributes addEntriesFromDictionary:attributeDict];
    FDXMLElement *p = [self.stack lastObject];
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string{
    FDXMLElement *elem = [self.stack lastObject];
    if (elem) {
...
-(NSString *) XMLString:(FDXMLElement *)elem level:(int)level{
    ...
    if (elem.sortedChildElements.count>0) {
        ...
        for (int k = 0; k<elem.sortedChildElements.count; k++) {
            FDXMLElement *e = [elem.sortedChildElements objectAtIndex:k];
            // invoke handler
