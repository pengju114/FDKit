...
@implementation FDHttpUtility

+(NSDictionary *)parseXML:(NSData *)data{
    FDXMLElement *xml = [[FDXMLElement alloc] initWithXMLData:data];
...
+(void) parseNodeTree:(NSMutableDictionary *) parent toTarget:(NSMutableDictionary *)element node:(FDXMLElement *) node {
    ...
    if ([node type] == kXMLElementNode) {
        ...
        if (list.count>1) {
            for (int i = 0; i < list.count; i++) {
                FDXMLElement *n = [list objectAtIndex:i];
...
+(BOOL) isMetaItemAndFetch:(FDXMLElement *) node toTarget:(NSMutableDictionary *) wrapper{
