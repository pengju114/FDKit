...
#import "FDCoreViewController.h"

#import "NSObject+FDApplicationSession.h"
#import "ASIFormDataRequest.h"
#import "FDHttpResult.h"
#import "FDHttpUtility.h"

#import "FDModalAlertDelegate.h"
...
@interface FDCoreViewController () <ASIHTTPRequestDelegate>
// http
...
@implementation FDCoreViewController
...
-(void)addToRequestQueue:(ASIHTTPRequest *)req{
    @synchronized(self){
        ...
        [self httpRequestCountDidChange:(int)_requestQueue.count increase:YES];
    }
}
...
-(void)removeFromRequestQueue:(ASIHTTPRequest *)req{
    @synchronized(self){
        ...
        [self httpRequestCountDidChange:(int)_requestQueue.count increase:NO];
    }
}
...
-(void)keyboardWillShow:(NSNotification *)ntf{
    ...
    FDLog(@"keyboardWillShow with size %@",NSStringFromCGSize(keyboardSize));
...
-(void)correctView{
    if (correctViewRef && _keyboardDidShow) {
        ...
        CGFloat screenHeight = [FDDeviceUtility screenHeight];
        
        FDLog(@"screenheight = %.1f rect in view %@",screenHeight,NSStringFromCGRect(rect));
        
        const CGFloat gap=0;// 加上gap在有多个输入框在滚动视图里面时，从上一输入框切换到下一输入框（此输入框往上滚才可见），有时会有露底的情况
        ...
        if (y > visibleHeight) {
            // 被挡
            _viewDidCorrect = YES;
            CGFloat mv = visibleHeight - y + [self extendedTop] + self.view.frame.origin.y; // 在原来的基础上上移
            if ([self.view respondsToSelector:@selector(constraints)]) {
                if(self.view.constraints.count > 0){
                    mv -= self.view.frame.origin.y; // 使用了 autolayout,不在原来基础上上移
                }
            }
            [self moveViewWithTop:mv];
        }
    }
}
...
-(void)moveViewWithTop:(CGFloat)top{
    ...
    FDLog(@"move view to %@",NSStringFromCGRect(n));
 ...
 */
-(BOOL) isHttpSuccessAndNotify:(FDHttpResult *)result{
    BOOL isOk = NO;
    if (result && result.statusCode == kHTTPOK) {
        isOk = YES;
    }else{
        [self showTip:result.statusText?result.statusText:@"http request failure"];
        isOk = NO;
    }
    return isOk;
}
 ...
 */
-(void)httpDidFailure:(ASIHTTPRequest *)req reason:(NSString *)reason{
 ...
 * 网络请求返回，属于UI线程
 */
-(void) httpDidResponse:(ASIHTTPRequest *)request withResult:(FDHttpResult *)result {
    
}

-(void) httpRequestCountDidChange:(int)count increase:(BOOL)increase{
    if (increase) {
...
- (void)request:(ASIHTTPRequest *)request didReceiveResponseHeaders:(NSDictionary *)responseHeaders{
    FDLog(@"http header:%@",responseHeaders);
}

- (void)requestFinished:(ASIHTTPRequest *)request{
    
    id responseData= [self makeResponseData:request];
    
    FDLog(@"cookie %@",request.responseCookies);
    
    if (responseData) {
        FDHttpResult *result = [[FDHttpResult alloc] initWithResponseData:responseData andResponseHeaders:[request responseHeaders]];
        dispatch_async(dispatch_get_main_queue(), ^{
            [self httpDidResponse:request withResult:result];
        });
    }else{
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *httpData = [[NSString alloc] initWithData:request.responseData encoding:request.defaultResponseEncoding];
            [self httpDidFailure:request reason:[NSString stringWithFormat:@"make data fail(%@)",AutoRelease(httpData)]];
        });
    }
...
- (void)requestFailed:(ASIHTTPRequest *)request{
    if ([request error].code==ASIConnectionFailureErrorType) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self httpDidFailure:request reason:[[request error] localizedDescription]];
        });
    }
    FDLog(@"http fail %@",[[request error] localizedDescription]);
    //收尾工作
    [self removeFromRequestQueue:request];
}


-(id)makeResponseData:(ASIHTTPRequest *)request{
    //处理数据
    NSData *data=request.responseData;
    
    //debug部分
    FDLog(@"http response:%@",AutoRelease([[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]));
    //debug end
    ...
    if (request.expectedDataFormat == kASIExpectedDataString) {
        ...
        responseData= AutoRelease(data);
    }else {
        if (request.responseDataFormat == kASIResponseDataJSON) {
            responseData=[FDHttpUtility parseJSON:data];
        }else if (request.responseDataFormat == kASIResponseDataXML) {
            responseData=[FDHttpUtility parseXML:data];
        }else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self httpDidFailure:request reason:@"illegal response data format"];
            });
        }
    }
...
+(void)enqueueModalAlertDelegate:(FDModalAlertDelegate *)delg{
...
+(BOOL)showConfirmMessage:(NSString *)title message:(NSString *)message yesButton:(NSString *)yesText cancelButton:(NSString *)cancelText delegate:(id<UIAlertViewDelegate>) delg{
    ...
    FDModalAlertDelegate *delegate = [[FDModalAlertDelegate alloc] initWithRunloop:CFRunLoopGetCurrent()];
    delegate.outsideDelegate = delg;
    
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:message delegate:delegate cancelButtonTitle:FDIsEmptyString(cancelText)?@"Cancel":cancelText otherButtonTitles:FDIsEmptyString(yesText)?@"OK":yesText, nil];
...
-(void)showProgressMessage:(NSString *)msg{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (progressCtr) {
            FDLog(@"show previous message view");
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (existMask) {
            ...
            if (![existMask containsView:view]) {// 之前没有添加到这里
                ...
                view.center = FDRectCenter(existCtr.frame);
                [existCtr addSubview:view];
            }
            existCtr.assistanceTag += 1;
            
            FDLog(@"progress exist");
        }else{
            FDLog(@"progress create");
            ...
            ctr.center = FDRectCenter(mask.frame);
            mask.center = FDRectCenter(self.view.frame);
...
-(void)closeProgressMessage{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (mask) {
            ...
            FDLog(@"prepare close progress,showing count %d",ctr.assistanceTag);
...
-(void)closeProgressAnimationFinish{
    ...
    if (mask) {
        AssistanceView *ctr = (AssistanceView *)[mask viewWithTag:kTagProgressMsgRoot];
        FDLog(@"set progress hidden count = %d",ctr.assistanceTag);
        if (ctr.assistanceTag < 1) {
            mask.hidden = YES;
        }
    }
...
-(void)showProgress:(BOOL)modal{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!ctr) {
            ...
            act.center = FDRectCenter(ctr.frame);
            
            [act startAnimating];
            [ctr addSubview:act];
            
            ctr.center = FDRectCenter(self.view.bounds);
            ctr.alpha = 0.0;
        ...
        FDLog(@"progress show count = %d",ctr.assistanceTag);
...
-(void)closeProgress{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (ctr) {
            ...
            FDLog(@"progress close count = %d",ctr.assistanceTag);
...
-(CGFloat)extendedTop{
    ...
    if ([self.view respondsToSelector:@selector(constraints)]) {
        if(self.view.constraints.count > 0){
            top = [self navigationBarHeight] + [self statusBarHeight];
        }else if (FDIOSVersion>=7.0){
...
-(CGFloat)statusBarHeight{
    ...
    return  (isLandscape? statusBarSize.width:statusBarSize.height);
}
